...
class CapacitorWifiConnect(context: Context) : LifecycleObserver {
  ...
  private val connectivityManager: ConnectivityManager by
          lazy(LazyThreadSafetyMode.NONE) {
            _context.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
          }

  private val locationManager: LocationManager by
          lazy(LazyThreadSafetyMode.NONE) {
            _context.getSystemService(Context.LOCATION_SERVICE) as LocationManager
          }

  private val wifiManager: WifiManager by
          lazy(LazyThreadSafetyMode.NONE) {
  ...
  fun connect(ssid: String, call: PluginCall) {
    ...
    ssid.let {
      when {
        Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q -> {
          val specifier = WifiNetworkSpecifier.Builder().setSsid(it).build()
  ...
  fun prefixConnect(ssid: String, call: PluginCall) {
        ...
        Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q -> {
          val specifier =
                  WifiNetworkSpecifier.Builder()
                          // .setBssidPattern(MacAddress.fromString("7c:df:a1:00:00:00"),
  ...
  fun connectByWifiNetworkSuggestionAndroid(ssid: String, password: String, call: PluginCall) {
    ...
    Log.d(TAG, "connectByWifiNetworkSuggestion: wifi=$ssid, pass=$password")
    val suggestion = WifiNetworkSuggestion.Builder().setSsid(ssid)
    ...
    wifiManager.removeNetworkSuggestions(
            suggestionsList
    ) // https://stackoverflow.com/questions/61295331/wifi-suggestion-api-fail-on-android
    ...
    Toast.makeText(_context, "result: $resultValue: $resultKey", Toast.LENGTH_SHORT).show()

    val intentFilter = IntentFilter(WifiManager.ACTION_WIFI_NETWORK_SUGGESTION_POST_CONNECTION)
    suggestionPostConnectionReceiver =
    ...
    _context.registerReceiver(suggestionPostConnectionReceiver, intentFilter)

    if (resultValue != STATUS_NETWORK_SUGGESTIONS_SUCCESS) {
      val ret = JSObject()
      ret.put("value", 0)
      call.resolve(ret)
    }
            ...
            object : BroadcastReceiver() {
              override fun onReceive(context: Context, intent: Intent) {
                ...
                if (success) {
                  ...
                  for (ssid in ssids) jsArray.put(ssid)
  ...
  @RequiresApi(Build.VERSION_CODES.M)
  private fun connectByPrefix(ssidPrefix: String, config: WifiConfiguration, call: PluginCall) {
    val wifiScanReceiver =
            object : BroadcastReceiver() {
              ...
              override fun onReceive(context: Context, intent: Intent) {
                ...
                if (success) {
                  ...
                  when {
                    ssid != null -> {
                      execConnect(config.apply { SSID = "\"" + ssid + "\"" }, call)
                    }
                    else -> {
                      val ret = JSObject()
                      ret.put("value", -1)
  ...
  private fun getNearbySsid(ssidPrefix: String): String? {
    ...
    return results
            .filter { scanResult -> scanResult.SSID.startsWith(ssidPrefix) }
            .maxByOrNull { scanResult -> scanResult.level }
            ?.SSID
  }
            ...
            object : BroadcastReceiver() {
              ...
              override fun onReceive(context: Context, intent: Intent) {
                ...
                if (info != null && info.isConnected) {
                  if ((info.extraInfo == wifiConfiguration.SSID) ||
                                  (getAppSSID() == wifiConfiguration.SSID)
                  ) {
                    val ret = JSObject()
                    ret.put("value", 0)
                    call.resolve(ret)
                    ...
                    // Ignore first callback if not success. It may be for the already connected
                    // SSID
                    val ret = JSObject()
                    ret.put("value", -1)
                    call.resolve(ret)
